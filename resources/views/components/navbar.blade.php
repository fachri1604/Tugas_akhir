<!-- Pastikan di <head> ada CSS x-cloak -->
<style>
...
<nav class="flex justify-between items-center px-6 py-4 border-b bg-white">
  ...
  <ul class="flex space-x-6 items-center text-sm">
    ...
    {{-- SEARCH (navbar) – tampil untuk semua user --}}
    <li x-data="{ open:false }" class="relative">
      <button @click="open = !open" class="hover:text-pink-500" aria-label="Cari">
      ...
      <div x-cloak x-show="open"
           ...
           class="absolute right-0 mt-2 bg-white border rounded shadow-lg p-3 w-72 z-50">
        <form action="{{ route('katalog') }}" method="GET" class="flex gap-2">
          <input type="text" name="q" value="{{ request('q') }}"
                 placeholder="Cari produk…"
                 class="flex-1 border rounded px-3 py-2 focus:outline-none focus:ring focus:border-pink-400">
          <button type="submit"
                  class="px-3 py-2 bg-pink-500 text-white rounded hover:bg-pink-600">
            Cari
          </button>
        </form>
      </div>
    ...
    @auth
      <li>
        <a href="{{ route('cart.index') }}" class="hover:text-pink-500 relative flex items-center" aria-label="Keranjang">
          <!-- Shopping bag icon -->
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
          </svg>
          @php
            $cartCount = method_exists(auth()->user(), 'cartItems')
    ...
    @auth
      <li class="relative" x-data="{ open: false }">
        ...
        <div x-cloak
             ...
             x-transition:leave-start="transform opacity-100 scale-100"
             x-transition:leave-end="transform opacity-0 scale-95"
             class="absolute right-0 mt-2 w-52 bg-white rounded-md shadow-lg py-1 z-50 border border-gray-100">
          ...
          <!-- Riwayat Pesanan -->
          <a href="{{ route('riwayat.index') }}" class="block px-4 py-2 text-sm hover:bg-pink-50 hover:text-pink-600">
